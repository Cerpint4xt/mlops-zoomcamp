import batch

import pandas as pd
from datetime import datetime
import pytest

def dt(hour, minute, second=0):
    return datetime(2023, 1, 1, hour, minute, second)

data = [
    (None, None, dt(1, 1), dt(1, 10)),
    (1, 1, dt(1, 2), dt(1, 10)),
    (1, None, dt(1, 2, 0), dt(1, 2, 59)),
    (3, 4, dt(1, 2, 0), dt(2, 2, 1)),
]

columns = ['PULocationID', 'DOLocationID', 'tpep_pickup_datetime', 'tpep_dropoff_datetime']
df = pd.DataFrame(data, columns=columns)

expected_data = [
    ('-1', '-1', dt(1, 1), dt(1, 10), 9.0),
    ('1', '1', dt(1, 2), dt(1, 10), 8.0)
]
expected_columns = ['PULocationID', 'DOLocationID', 'tpep_pickup_datetime', 'tpep_dropoff_datetime', 'duration']
expected_df = pd.DataFrame(expected_data, columns=expected_columns)

def test_prepare_data():
    from batch import prepare_data  # Replace with the actual script name
    categorical = ['PULocationID', 'DOLocationID']
    actual_df = prepare_data(df, categorical)
    print(actual_df.shape)
    # Convert DataFrame to list of dictionaries for comparison
    actual_dicts = actual_df.to_dict(orient='records')
    expected_dicts = expected_df.to_dict(orient='records')
    
    assert actual_dicts == expected_dicts